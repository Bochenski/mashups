...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  ret.hid_to_class = ((class_prob - data.targets) * B') / N + wd_coefficient * Theta;
  ret.input_to_hid = (((Theta' * (Y - data.targets)) .* B .* (1-B))*X')/ N + wd_coefficient * model.input_to_hid;
end
